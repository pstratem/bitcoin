...
constexpr uint32_t WALLET_FILTER_M = 1.497137 * pow(2,WALLET_FILTER_P);
...
struct DBVal {
    ...
    SERIALIZE_METHODS(DBVal, obj) { READWRITE(obj.pos, obj.filter_hash); }
};
...
bool WalletFilterIndex::CustomAppend(const interfaces::BlockInfo& block)
{
    ...
    uint64_t filter_hash = CSipHasher(m_params.m_siphash_k0, m_params.m_siphash_k1)
...
bool WalletFilterIndex::LookupFilter(const CBlockIndex* block_index, GCSFilter& filter_out) const
{
    ...
    return filter_hash == entry.filter_hash;
}
